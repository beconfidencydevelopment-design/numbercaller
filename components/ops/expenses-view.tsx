...
function DayHeader({ at, rows }: { at: number; rows: ExpenseEntry[] }) {
  ...
  return (
    <tr className="bg-ops-sunken">
      <td colSpan={5} className="border-y border-ops-line px-4 py-1.5">
      ...
      <td className="border-y border-ops-line px-3 py-1.5 text-right">
...
function QuietDay({ at }: { at: number }) {
  return (
    <tr>
      <td colSpan={7} className="border-b border-ops-line px-4 py-1.5">
    ...
    <>
      ...
      <PageBody className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <label className="relative flex h-8 min-w-[190px] flex-1 items-center sm:max-w-[260px]">
            <Search className="pointer-events-none absolute left-2.5 size-3.5 text-ops-text-tertiary" />
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search entries"
              aria-label="Search entries"
              className="h-full w-full rounded-[var(--ops-r-control)] border border-ops-line bg-ops-surface pl-8 pr-2.5 text-[13px] text-ops-text outline-none placeholder:text-ops-text-tertiary focus:border-ops-line-strong"
            />
          </label>
          ...
          <div role="radiogroup" aria-label="Status" className="flex h-8 items-center rounded-[var(--ops-r-control)] border border-ops-line bg-ops-surface p-0.5">
            {([
              ["all", "All"],
              ...
              <table className="w-full min-w-[820px] border-collapse text-[13px]">
                ...
                <tfoot>
                  <tr className="border-t-2 border-ops-line-strong bg-ops-sunken">
                    <td colSpan={5} className="px-4 py-2.5 text-[12px] font-semibold text-ops-text">
                    ...
                    <td className="px-4 py-2.5 text-right text-[11px] text-ops-text-tertiary">
      ...
      {selected.size > 0 && (
        <div className="pointer-events-none sticky bottom-3 z-10 flex justify-center px-5">
          <div className="pointer-events-auto flex items-center gap-3 rounded-full border border-ops-line bg-ops-surface px-3 py-1.5 shadow-ops-pop">
            <span className="text-[12px] text-ops-text-secondary">
