...
function NavLink({ item, compact }: { item: NavItem; compact?: boolean }) {
  ...
  return (
    <Link
      ...
      className={cn(
        "relative flex h-9 items-center gap-3 rounded-[var(--ops-r-control)] px-3 text-body transition-colors",
        active
    ...
    >
      ...
      {item.count !== undefined && item.count > 0 && (
        <span className="ops-num ml-auto text-body text-ops-text-tertiary">{item.count}</span>
      )}
    </Link>
  );
}
    ...
    <div className="ops-root ops-frame">
      <div className="flex">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <footer className="ops-num flex h-8 shrink-0 items-center gap-3 border-t border-ops-line bg-ops-surface px-6 text-body text-ops-text-tertiary">
            <span>
